M.allele.seq.variant<-function(sampleDir="./samplesF.MA",AlleleList,locusInfo,alleleDB, HeteroThreshold=0.4,
	...
	coverageF<-CalibrationFile[nrow(CalibrationFile),]
	...
	for(s in 1:nsamples){
		...
		for(p in 1:nprimers){
			...
			Alleles.by.Size<-AlleleList[samples[s]][[1]][[p]] # retrieve the alleles found for locus p in sample s
			nalleles<-length(Alleles.by.Size)
			if(nalleles==0){
				SEQ.ALLELES.Loci<-append(SEQ.ALLELES.Loci,list(NULL))
				next
			}
