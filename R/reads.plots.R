reads.plots<-function(sampleDir="./samplesF",locusInfo="primers",x.range=c(90,350),x.label.cex=0.8,RepNumberPlot=FALSE ){
  ...
  print(paste0("Processing sample plots for locus number ",p," : ",primers$Locus[p]))
  pdf(file=paste0("./Plots/Plot.",primers[p,1],".pdf"),width=11,height=5)
   for(f in 1:nfiles){   
     ...
     hist(DF.sample.read.msat.sizes$ReadSize,breaks=seq(0,xlimU,1),xlim=c(xlimL,xlimU),axes=F,
    	main=paste0(files[f],":",primers[p,1]),xlab="Read size (bp)")
      par(cex=x.label.cex)
